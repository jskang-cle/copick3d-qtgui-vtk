...
#include "vtkCameraOrientationWidgetEx.h"
#include "vtkAbstractWidget.h"
#include "vtkCallbackCommand.h"
#include "vtkCamera.h"
#include "vtkCameraInterpolator.h"
#include "vtkCameraOrientationRepresentationEx.h"
...
vtkStandardNewMacro(vtkCameraOrientationWidgetEx);

//----------------------------------------------------------------------------
vtkCameraOrientationWidgetEx::vtkCameraOrientationWidgetEx()
{
  // Define widget events.
  this->CallbackMapper->SetCallbackMethod(vtkCommand::LeftButtonPressEvent, vtkEvent::NoModifier, 0,
    0, nullptr, vtkWidgetEvent::Select, this, vtkCameraOrientationWidgetEx::SelectAction);
  this->CallbackMapper->SetCallbackMethod(vtkCommand::LeftButtonReleaseEvent, vtkEvent::NoModifier,
    0, 0, nullptr, vtkWidgetEvent::EndSelect, this, vtkCameraOrientationWidgetEx::EndSelectAction);
  this->CallbackMapper->SetCallbackMethod(vtkCommand::MouseMoveEvent, vtkEvent::NoModifier, 0, 0,
    nullptr, vtkWidgetEvent::Rotate, this, vtkCameraOrientationWidgetEx::MoveAction);
...
//----------------------------------------------------------------------------
vtkCameraOrientationWidgetEx::~vtkCameraOrientationWidgetEx() = default;

//------------------------------------------------------------------------------
vtkRenderer* vtkCameraOrientationWidgetEx::GetParentRenderer()
{
  return this->ParentRenderer;
}

//------------------------------------------------------------------------------
void vtkCameraOrientationWidgetEx::SetDefaultRenderer(vtkRenderer* renderer)
{
  ...
  this->ReorientObserverTag = renderer->AddObserver(
    vtkCommand::StartEvent, this, &vtkCameraOrientationWidgetEx::OrientWidgetRepresentation);
...
//------------------------------------------------------------------------------
void vtkCameraOrientationWidgetEx::SetParentRenderer(vtkRenderer* parentRen)
{
  ...
  if (parentRen != nullptr)
  {
    ...
    if (renWin != nullptr)
    {
      ...
      this->ResizeObserverTag = renWin->AddObserver(
        vtkCommand::StartEvent, this, &vtkCameraOrientationWidgetEx::SquareResize);
    }
  }
...
//----------------------------------------------------------------------------
void vtkCameraOrientationWidgetEx::SetRepresentation(vtkCameraOrientationRepresentationEx* r)
{
  this->Superclass::SetWidgetRepresentation(r);
}

//----------------------------------------------------------------------------
void vtkCameraOrientationWidgetEx::CreateDefaultRepresentation()
{
  if (!this->WidgetRep)
  {
    this->WidgetRep = vtkCameraOrientationRepresentationEx::New();
  }
}

void vtkCameraOrientationWidgetEx::ComputeWidgetState(int X, int Y, int modify /* =0*/)
{
  auto rep = vtkCameraOrientationRepresentationEx::SafeDownCast(this->WidgetRep);
  if (rep == nullptr)
  {
    return;
  }
  ...
  if (interactionState == vtkCameraOrientationRepresentationEx::InteractionStateType::Outside)
  {
    this->WidgetState = WidgetStateType::Inactive;
  }
  else if (interactionState == vtkCameraOrientationRepresentationEx::InteractionStateType::Hovering)
  {
...
//----------------------------------------------------------------------------
void vtkCameraOrientationWidgetEx::SelectAction(vtkAbstractWidget* w)
{
  // cast to ourself
  vtkCameraOrientationWidgetEx* const self = vtkCameraOrientationWidgetEx::SafeDownCast(w);
  if (self == nullptr)
  {
    return;
  }
...
//----------------------------------------------------------------------------
void vtkCameraOrientationWidgetEx::EndSelectAction(vtkAbstractWidget* w)
{
  // cast to ourself
  vtkCameraOrientationWidgetEx* const self = vtkCameraOrientationWidgetEx::SafeDownCast(w);
  if (self == nullptr)
  ...
  auto rep = vtkCameraOrientationRepresentationEx::SafeDownCast(self->WidgetRep);
  if (rep == nullptr)
  {
    return;
  }
  ...
  // vtkLog(INFO, "EndSelectAction at " << X << "," << Y);
  ...
  if (rep->IsAnyHandleSelected() &&
    (rep->GetInteractionStateAsEnum() ==
      vtkCameraOrientationRepresentationEx::InteractionStateType::Hovering))
  {
    double back[3], up[3];
    rep->GetBack(back);
    rep->GetUp(up);
...
//----------------------------------------------------------------------------
void vtkCameraOrientationWidgetEx::StartAnimation()
{
  this->AnimatorCurrentFrame = 1;
  this->AnimationTimerId = this->Interactor->CreateRepeatingTimer(10);
  this->AnimationTimerObserverTag = this->Interactor->AddObserver(
    vtkCommand::TimerEvent, this, &vtkCameraOrientationWidgetEx::PlayAnimationSingleFrame);
}

//----------------------------------------------------------------------------
void vtkCameraOrientationWidgetEx::PlayAnimationSingleFrame(
...
//----------------------------------------------------------------------------
void vtkCameraOrientationWidgetEx::StopAnimation()
{
...
//----------------------------------------------------------------------------
void vtkCameraOrientationWidgetEx::MoveAction(vtkAbstractWidget* w)
{
  // cast to ourself
  vtkCameraOrientationWidgetEx* const self = vtkCameraOrientationWidgetEx::SafeDownCast(w);
  if (self == nullptr)
...
//-----------------------------------------------------------------------------
void vtkCameraOrientationWidgetEx::OrientParentCamera(double back[3], double up[3])
{
  if (this->ParentRenderer == nullptr)
  {
    return;
  ...
  vtkNew<vtkCamera> targetCam;
  // targetCam->DeepCopy(cam);

  // set new camera vars
  targetCam->SetFocalPoint(focalP);
  targetCam->SetPosition(dstPos);
  targetCam->SetViewUp(up);
  targetCam->ComputeViewPlaneNormal();

  this->CameraInterpolator->AddCamera(this->AnimatorTotalFrames - 1, targetCam);
}

//-----------------------------------------------------------------------------
void vtkCameraOrientationWidgetEx::OrientWidgetRepresentation()
{
  if (this->ParentRenderer == nullptr)
  {
    return;
...
//-----------------------------------------------------------------------------
void vtkCameraOrientationWidgetEx::InterpolateCamera(int t)
{
  if (this->ParentRenderer == nullptr)
  {
    return;
...
//----------------------------------------------------------------------------
void vtkCameraOrientationWidgetEx::SquareResize()
{
  if (this->DefaultRenderer == nullptr)
  {
    return;
  ...
  switch (anchoredTo)
  {
    case vtkCameraOrientationRepresentationEx::AnchorType::LowerLeft:
      xmin = 0. + vppadw;
      xmax = vpw + vppadw;
      ymin = 0. + vppadh;
      ymax = vph + vppadh;
      break;
    case vtkCameraOrientationRepresentationEx::AnchorType::LowerRight:
      xmin = 1. - vpw - vppadw;
      xmax = 1. - vppadw;
      ymin = 0. + vppadh;
      ymax = vph + vppadh;
      break;
    case vtkCameraOrientationRepresentationEx::AnchorType::UpperLeft:
      xmin = 0.0 + vppadw;
      xmax = vpw + vppadw;
      ymin = 1. - vph - vppadh;
      ymax = 1. - vppadh;
      break;
    case vtkCameraOrientationRepresentationEx::AnchorType::UpperRight:
      xmin = 1. - vpw - vppadw;
      xmax = 1. - vppadw;
      ymin = 1. - vph - vppadh;
      ymax = 1. - vppadh;
...
//----------------------------------------------------------------------------
void vtkCameraOrientationWidgetEx::PrintSelf(ostream& os, vtkIndent indent)
{
